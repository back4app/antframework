...
class Ant {
  ...
  constructor(config) {
     ...
     * @private
     */
    this._functionController = new FunctionController(this);
  }
  ...
  */
  _getGlobalConfig() {
    ...
    try {
      const configFileContent = fs.readFileSync(globalConfigPath, 'utf8');
      globalConfig = configFileContent ? yaml.parse(
   ...
   */
  get runtime() {
    return this.functionController.getFunction('runtime');
  }
  ...
  async addPlugin(plugin, isGlobal) {
    try {
      ...
      throw new AntError('Plugin could not be added', e);
    }
  }
