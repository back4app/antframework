...
template remove command should do nothing`);
  ...
  addFunction(antFunction) {
    ...
    if (!functions) {
      functions = new Pair(new Scalar('functions'), new Seq());
      this._config.contents.items.push(functions);
    }
    if (functions.value && functions.value.items && functions.value.items.find(
      func => func.items && func.items.find(property => property.key.value === 'name' && property.value.value === name)
    )) {
      console.log(`Function "${name}" already found on current config. \
function add command should do nothing`);
      return this;
    }
    logger.log(`Adding function ${name} into configuration file ${this._path}`);
    const newFunctionNode = new Map();
    newFunctionNode.items.push(new Pair(new Scalar('name'), new Scalar(name)));
    if (antFunction instanceof BinFunction) {
      newFunctionNode.items.push(new Pair(new Scalar('bin'), new Scalar(bin)));
    } else {
      newFunctionNode.items.push(new Pair(new Scalar('handler'), new Scalar(handler)));
      newFunctionNode.items.push(new Pair(new Scalar('runtime'), new Scalar(runtime.name)));
    }
    functions.value.items.push(newFunctionNode);
  ...
  removeFunction(antFunction) {
    ...
    const functions = this._config.contents.items.find(item => item.key.value === 'functions');
    if (!functions || !functions.value || !functions.value.items) {
      console.log('No functions was found on configuration file. function \
remove command should do nothing');
      return this;
    }
    if (!functions.value.items.find(item =>
      item.items.find(
        property => property.key.value === 'name' && property.value.value === antFunction
      )
    )) {
...
provider "${providerName}"`
   ...
   *
   * @param {Array} functions An array of AntFunctions from the configuration file.
  ...
  static ParseConfigFunctions(functions, runtimeController) {
    ...
    return functions.map(func => {
      const { name, bin, handler, runtime } = func;
      try {
        if (bin) {
          return new BinFunction(runtimeController.ant, name, bin);
        } else if (handler && runtime) {
          return new LibFunction(runtimeController.ant, name, handler,
            runtimeController.getRuntime(runtime));
        }
