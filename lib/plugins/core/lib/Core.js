  ...
  constructor(ant, config) {
    ...
    this._runtimes = [
      new Runtime(
        ...
        ['js'],
        path.resolve(__dirname, '../templates/runtime/node.js.mustache')
      )
    ];
  }
...
using template "${argv.template}"`
      ...
      'Manage functions of Ant framework', yargs => {
        yargs.command(
          'add <name> [function] [runtime] [--global]',
...
Considering "${template}" as the template files path.`);
  ...
  async addFunction(name, func, runtime, type = 'lib', isGlobal) {
    ...
    case 'lib':
      ...
      // If function file does not exists, and the runtime has a
      // template for new functions, we should use it to render
      // this new function file.
      if (!fs.existsSync(func) && runtimeInstance.template) {
        let templateFileContent = fs.readFileSync(runtimeInstance.template, { encoding: 'utf-8' });
        templateFileContent = Mustache.render(templateFileContent, { name });
        fs.ensureFileSync(func);
        fs.writeFileSync(func, templateFileContent);
      }
