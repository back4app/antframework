...
using template "${argv.template}"`
      ...
      'Manage functions of Ant framework', yargs => {
        yargs.command(
          ...
          async argv => {
            try {
              const { name, function: func, runtime, global } = argv;
              await this.addFunction(name, func, runtime, global);
              process.exit(0);
            } catch (e) {
          ...
          'Executes a function', yargs => {
            yargs.positional('function', {
              ...
              alias: 'p',
              describe: 'One or more execution parameters',
              type: 'array'
            });
          },
          async argv => {
            try {
              const observable = await this.execFunction(argv.function, argv.params);
              const onNext = data => {
                console.log(data);
              };
              const onError = err => {
...
Considering "${template}" as the template files path.`);
  ...
  async addFunction(name, func, runtime, isGlobal) {
    const config = Core._getConfig(isGlobal);
    if (runtime) {
      config.addFunction(new LibFunction(this.ant, name, func, runtime));
    } else {
