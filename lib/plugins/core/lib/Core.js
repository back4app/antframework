        ...
        yargs.command(
          ...
          'Installs new plugin', yargs => {
            yargs.positional('plugin', {
              ...
              alias: 'g',
              describe: 'Installs plugin into global configuration file',
              boolean: true,
              nargs: 0,
              default: false
            });
          ...
          async (argv) => {
            try {
              ...
              console.log(`Plugin "${argv.plugin}" successfully removed ${argv.global ? 'globally' : ''}`);
              process.exit(0);
            } catch (e) {
...
configuration file`);
  ...
  async removePlugin(plugin, isGlobal) {
    ...
    if (!config || !config.contents) {
      logger.log('Configuration not found. plugin remove command should do nothing');
      return null;
    }
    ...
    if (!plugins || !plugins.value || !plugins.value.items) {
      logger.log(`No plugins installed was found on ${isGlobal ? 'global' : 'local'} \
configuration file. plugin remove command should do nothing`);
      return null;
    }
    if (!plugins.value.items.find(item => item.value === plugin)) {
      logger.log(`Plugin "${plugin}" is already uninstalled from the ${isGlobal ? 'global' : 'local'} \
configuration file. plugin remove command should do nothing`);
      return null;
    }
    ...
    logger.log(`Plugin "${plugin}" uninstalled successfully from the ${isGlobal ? 'global' : 'local'} \
configuration file`);
