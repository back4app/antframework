...
describe('GraphQL Server - lib/Server.js', () => {
  ...
  test('should load the GraphQL schema', () => {
    ...
    process.chdir(path.resolve(__dirname, '../support/resolvers'));
    ...
    expect(graphql(server._schema, '{ foo }')).resolves.toEqual(
      { data: { foo: 'foo resolver value' } }
    );
    expect(graphql(server._schema, '{ mocked }')).resolves.toEqual(
      { data: { mocked: 'foo mock value' } }
    );
    expect(graphql(server._schema, '{ mocked2 }')).resolves.toEqual(
      { data: { mocked2: null } }
    );
    expect(graphql(server._schema, '{ mocked3 }')).resolves.toEqual(
      { data: { mocked3: null } }
    );
